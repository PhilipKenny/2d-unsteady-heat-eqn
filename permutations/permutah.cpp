...
//    cout << nprm << endl; 
 //   cout << sizeof(int)*nn <<endl;
    for (size_t in = 0; in < nn; ++in)
    {
        coordneu[in*nsd] = coord[nperm[in]*nsd]; 
        coordneu[in*nsd+1] = coord[nperm[in]*nsd+1]; 
        cout << nperm[in] << endl;
    }

    for (size_t ie = 0; ie < ne; ++ie)
    {
        connneu[ie*nen] = conn[mperm[ie]*nen]; 
        connneu[ie*nen+1] = conn[mperm[ie]*nen+1]; 
        connneu[ie*nen+2] = conn[mperm[ie]*nen+2]; 

        facerneu[ie*nen] = facer[mperm[ie]*nen]; 
        facerneu[ie*nen+1] = facer[mperm[ie]*nen+1]; 
        facerneu[ie*nen+2] = facer[mperm[ie]*nen+2]; 
        cout << mperm[ie] << endl;
    }
    ...
    utility::swapBytes((char*)connneu, nen*ne, sizeof(int));
    dummy = pwd + mien_neu + int2str.str();
    ofstream fout2(dummy.c_str(), ios::out|ios::binary);
    fout2.write((char*)connneu, sizeof(int)*ne*nen);
    fout2.close();
